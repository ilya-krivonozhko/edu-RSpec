...
RSpec.describe ExchangeIt::Account do
  ...
  describe '#deposit', :fast do
    ...
    it 'allows to deposit negative sum' do
      expect { ann.withdraw(-100) }.to raise_error(ExchangeIt::IncorectSum, /be positive/)
    end

    it 'allows to deposit zero sum' do
      expect { ann.withdraw(0) }.to raise_error(ExchangeIt::IncorectSum, /be positive/)
    end
  end
  ...
  context 'when performing money withdrawal' do
    ...
    specify '#transfer_with_convertion' do
    ...
    specify '#transfer (mocked)', :linux_only do
      ann_mocked = instance_double(described_class, balance: 30, deposit: 30)
    ...
    describe '#withdraw' do
      ...
      it 'does not allow to withdraw a negative sum' do
        expect { john.withdraw(-10) }.to raise_error(ExchangeIt::IncorectSum, /be positive/)
      end

      it 'does not allow to withdraw a zero sum' do
        expect { john.withdraw(0) }.to raise_error(ExchangeIt::IncorectSum, /be positive/)
      end
    end
  end
end
